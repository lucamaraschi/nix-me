...
# Simple fallback UI functions if ui.sh doesn't exist yet
...
main() {
    ...
    # STEP 1: Clone/update repository FIRST (before wizard)
    print_step "1/6" "Setting up configuration repository"
    if [ -d "$REPO_DIR" ]; then
    ...
    # STEP 2: NOW source UI library from the cloned repo
    if [ -f "$REPO_DIR/lib/ui.sh" ]; then
        source "$REPO_DIR/lib/ui.sh"
    fi

    # STEP 3: Run wizard if appropriate (now that repo exists)
    ...
    # Request sudo
    print_step "2/6" "Requesting Administrator Privileges"
    ...
    print_step "3/6" "Installing Xcode Command Line Tools"
