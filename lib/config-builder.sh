...
# New function: generate config with profile support
generate_machine_config_with_profile() {
    ...
    print_info "Generating configuration for $hostname with profile: $profile"

    # Create machine-specific directory
    local host_dir="$repo_dir/hosts/$hostname"
    mkdir -p "$host_dir"

    # Generate host config based on profile
    if [ "$profile" != "minimal" ] && [ "$profile" != "custom" ]; then
...

{
  ...
  apps = {
    # You can add more apps specific to this machine
    casksToAdd = [
      # Add your machine-specific apps here
    ];

    casksToRemove = [
      # Remove unwanted apps here
    ];
  };
...
EOF
        ...
        # Minimal or custom profile - generate basic config
        cat > "$host_dir/default.nix" << EOF
{ config, pkgs, lib, ... }:
...
EOF
    ...
    # Update flake.nix with profile
    update_flake_with_machine_and_profile "$repo_dir" "$hostname" "$machine_type" "$machine_name" "$username" "$profile"
}
...
update_flake_with_machine_and_profile() {
    local repo_dir="$1"
    local hostname="$2"
    ...
    # Create the machine definition with profile
    local machine_def
    if [ "$profile" != "minimal" ] && [ "$profile" != "custom" ]; then
